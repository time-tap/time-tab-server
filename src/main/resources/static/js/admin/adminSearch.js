...
function setSearchFilter() {
	...
	// 체크된 상태 값 가져오기
	searchFilter.auth = $('input[name="auth"]:checked').val();
...
function getAdminUserList() {
	...
	// api 요청
	$.ajax({
		url: '/admin/api/admin/getAdminUserList',
		method: 'POST',
		contentType: 'application/json',
		data: JSON.stringify(searchFilter),
		success: function(response) {
			console.log('adminUserList: ', response.users);
			resultUpdate(response); // 검색결과 업데이트
...
function resultUpdate(data) {
	...
	const adminUserList = data.users;
	...
	if (adminUserList && adminUserList.length > 0) {
		// 검색 결과가 있을 경우
		adminUserList.forEach(function(user, index) {
...
function resultRaw(user, index) {
	...
	// 유저 권한 객체 리스트
	const auths = user.authList;
	
	// ROLE_ADMIN_MASTER 존재 여부 체크
    let roleLabel = "일반 관리자"; // 기본값
    if (auths && auths.length > 0) {
        for (let i = 0; i < auths.length; i++) {
            if (auths[i].authNm === 'ROLE_ADMIN_MASTER') {
                roleLabel = "최고 관리자";
	...
	// authDesc 모으되 ROLE_ADMIN, ROLE_ADMIN_MASTER는 제외
    let authDescList = [];
    if (auths && auths.length > 0) {
        for (let i = 0; i < auths.length; i++) {
            const auth = auths[i];
            if (auth.authNm !== 'ROLE_ADMIN' && auth.authNm !== 'ROLE_ADMIN_MASTER') {
                if (auth.authDesc) {
                    authDescList.push(auth.authDesc);
                }
            }
        }
    }
    const authDescStr = authDescList.join(' / ');
	
	let resultRaw = `
		<div class="result-body-raw">
            <div class="result-body-column column1" style="width: 40px;">
                <input type="checkbox" name="user" class="user" data-id="${user.userIdx}" data-idx="${user.userIdx}">
            </div>
            <div class="result-body-column column2" style="width: 60px;">
                <span>${startIndex + index}</span>
            </div>
            <div class="result-body-column column3" style="width: 120px;">
                <span>${user.userId}</span>
            </div>
            <div class="result-body-column column4" style="width: 100px;">
                <span>${user.userNm}</span>
            </div>
            <div class="result-body-column column5" style="width: 100px;">
                <span>${roleLabel}</span>
            </div>
            <div class="result-body-column column6" style="width: 290px;">
                <span>${authDescStr}</span>
            </div>
            <div class="result-body-column column7" style="width: 120px;">
				<span>${user.userTel}</span>
			</div>
            <div class="result-body-column column8" style="width: 90px;">
				<span>${formatDate(user.regDt)}</span>
			</div>
            <div class="result-body-column column9" style="width: 70px;">
				<button class="info-btn" data-idx="${user.userIdx}">
					<span class="blue">상세</span>
				</button>
	        </div>
			<div class="result-body-column column10 fixed" style="width: 70px;">
				<button class="del-btn" data-idx="${user.userIdx}">
...
// 선택한 관리자 계정 삭제 버튼 클릭 이벤트
$('#all-del-btn').on('click', function() {
	// 선택된 체크박스를 가져옴
	const selectedUser = $('#result-body .user:checked');
	...
	const confirmed = confirm('정말로 선택한 계정을 삭제 하시겠습니까?');
	...
	selectedUser.each(function() {
		...
		// 객체에 추가
		requestData.userIdx.push(userIdx);
	});


	console.log('삭제할 계정들: ', requestData.userIdx);

	// 삭제 요청
	removeAdminUser(requestData);

});

// 개별 회원 탈퇴 버튼 클릭 이벤트
$('#result-body').on('click', '.del-btn', function() {
	const userIdx = $(this).data('idx'); // 선택한 자문 인덱스
	let confirmed = confirm(`정말로 계정을 삭제 하시겠습니까?`);
...
// 탈퇴 서버 요청 함수
function removeAdminUser(requestData) {
	// api 요청
	$.ajax({
		url: '/admin/api/admin/removeAdminUser', // 서버의 삭제 API URL
		type: 'POST',
		contentType: 'application/json',
...
$('#result-body').on('click', '.info-btn', function() {
	...
	// URL
	const url = '/admin/adminUserDetail?userIdx=' + userIdx;
