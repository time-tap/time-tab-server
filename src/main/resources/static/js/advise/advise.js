...
// 페이지 진입시 검색 
getAdviseList(searchFilter); // 페이지 진입시 전체 검색 결과 출력
...
// 사용자 유형 "전체" 체크박스 클릭 이벤트
$('#user-type-all').on('change', function() {
	const isChecked = $(this).is(':checked');

	// 다른 체크박스들의 상태를 "전체" 체크박스와 동일하게 설정
	$('input[type=checkbox][name=user-type]').prop('checked', isChecked);

	// span 태그에 bold700 클래스 추가/제거
	$('.user-type-label-text').toggleClass('bold700', isChecked);

});

// 사용자 유형 다른 체크박스 클릭 이벤트
$('input[type=checkbox][name=user-type]').on('change', function() {
	const checkboxIndex = $('input[type=checkbox][name=user-type]').index(this);
	// 현재 체크박스와 동일한 인덱스의 span 태그에 bold700 클래스 추가/제거
	$('.user-type-label-text').eq(checkboxIndex + 1).toggleClass('bold700', $(this).is(':checked'));

	// 모든 다른 체크박스가 체크되어 있는지 확인
	const allChecked = $('input[type=checkbox][name=user-type]').length === $('input[type=checkbox][name=user-type]:checked').length;

	// "전체" 체크박스의 상태를 업데이트
	$('#user-type-all').prop('checked', allChecked);
	$('.user-type-all-label .user-type-label-text').toggleClass('bold700', allChecked);
});

// 전문가 유형 "전체" 체크박스 클릭 이벤트
$('#adviser-type-all').on('change', function() {
	const isChecked = $(this).is(':checked');

	// 다른 체크박스들의 상태를 "전체" 체크박스와 동일하게 설정
	$('input[type=checkbox][name=adviser-type]').prop('checked', isChecked);

	// span 태그에 bold700 클래스 추가/제거
	$('.adviser-type-label-text').toggleClass('bold700', isChecked);

});

// 전문가 유형 다른 체크박스 클릭 이벤트
$('input[type=checkbox][name=adviser-type]').on('change', function() {
	const checkboxIndex = $('input[type=checkbox][name=adviser-type]').index(this);
	// 현재 체크박스와 동일한 인덱스의 span 태그에 bold700 클래스 추가/제거
	$('.adviser-type-label-text').eq(checkboxIndex + 1).toggleClass('bold700', $(this).is(':checked'));

	// 모든 다른 체크박스가 체크되어 있는지 확인
	const allChecked = $('input[type=checkbox][name=adviser-type]').length === $('input[type=checkbox][name=adviser-type]:checked').length;

	// "전체" 체크박스의 상태를 업데이트
	$('#adviser-type-all').prop('checked', allChecked);
	$('.adviser-type-all-label .adviser-type-label-text').toggleClass('bold700', allChecked);
});
...
$('#reset-btn').on('click', function() {
	...
	// 사용자 유형 체크박스 초기화 (모두 체크)
	$('#user-type-all').prop('checked', true); // "전체" 체크
	$('input[type=checkbox][name=user-type]').prop('checked', true); // 모든 유형 체크

	// 스타일 초기화
	$('.user-type-label-text').addClass('bold700'); // 모든 유형 bold 처리
	$('.user-type-all-label .user-type-label-text').addClass('bold700'); // "전체" bold 처리

	// 전문가 유형 체크박스 초기화 (모두 체크)
	$('#adviser-type-all').prop('checked', true); // "전체" 체크
	$('input[type=checkbox][name=adviser-type]').prop('checked', true); // 모든 유형 체크

	// 스타일 초기화
	$('.adviser-type-label-text').addClass('bold700'); // 모든 유형 bold 처리
	$('.adviser-type-all-label .adviser-type-label-text').addClass('bold700'); // "전체" bold 처리
...
// 전체 체크박스 이벤트 (이벤트 위임)
$('#result-container').on('change', '.advise-all', function() {
	const isChecked = $(this).is(':checked');
	$('#result-body .advise').prop('checked', isChecked);
});

// 개별 체크박스 이벤트 (이벤트 위임)
$('#result-container').on('change', '.advise', function() {
	const total = $('#result-body .advise').length; // 전체 체크박스 개수
	const checked = $('#result-body .advise:checked').length; // 체크된 체크박스 개수
	// 모든 .user 체크박스가 체크되었는지 확인
	$('#result-head .advise-all').prop('checked', total === checked);
});

// 상태 변경 버튼 클릭
$('#all-change-btn').on('click', function() {
	// 상태 리스트 출력 토글
	$('.all-change-li-container').toggleClass('open');
})

// 상태 변경 버튼 밖에 클릭 시 닫기
$('#wrap').on('click', function(event) {
	if (!$(event.target).closest('.all-change-li-container, #all-change-btn').length) {
		$('.all-change-li-container').removeClass('open');
	}
});

// 선택한 자문 상태 변경 클릭
$('.all-change-li').on('click', function() {
	// 선택된 체크박스를 가져옴
	const selectedAdvise = $('#result-body .advise:checked');

	if (selectedAdvise.length === 0) {
		alert('자문을 선택하세요.');
		// 상태 변경 리스트 창 닫기
		$('.all-change-li-container').removeClass('open');
		return;
	...
	const confirmed = confirm(`선택한 ${selectedAdvise.length}개의 자문을 ${statusNm}(으)로 변경하시겠습니까?`);
	...
	// 선택된 자문 목록을 반복 처리
	selectedAdvise.each(function() {
		const adviseIdx = $(this).data('idx'); // 체크박스의 data-idx 속성에서 회원 idx 가져오기

		// 객체에 추가
		request.adviseIdxList.push(adviseIdx);
	});

	// 상태 변경 리스트 창 닫기
	$('.all-change-li-container').removeClass('open');

	console.log('변경할 자문들: ', request);

	// 변경 요청
	updateAdvise(request);

});

// 상태 변경 요청 API
function updateAdvise(request) {
	// api 요청
	$.ajax({
		url: '/admin/api/advise/updateAdvise',
		type: 'POST',
		...
		success: function(response) {
			...
			if (response.success) {
				...
				getAdviseList(searchFilter); // 검색 요청
			} else {
				alert(response.message);
			}
		},
...
$('#all-del-btn').on('click', function() {
	...
	const confirmed = confirm('정말로 선택한 자문들을 삭제 하시겠습니까?');
	...
	console.log('삭제할 자문들: ', request);

	// 삭제 요청
	removeAdvise(request);

});

// 개별 회원 탈퇴 버튼 클릭 이벤트
$('#result-body').on('click', '.del-btn', function() {
	const adviseIdx = $(this).data('advise-idx'); // 선택한 자문 인덱스
	let confirmed = confirm(`정말로 자문을 삭제 하시겠습니까?`);
...
function removeAdvise(request) {
	...
	$.ajax({
		...
		error: function() {
			console.log('removeAdvise AJAX 요청 실패');
		}
	});
}
...
function setSearchFilter() {
	...
	// 체크된 사용자 유형 값 가져오기 (여러 개 선택 가능할 경우 배열로 설정)
	searchFilter.userTypes = $('input[name="user-type"]:checked').map(function() {
		return $(this).val();
	}).get();

	// 체크된 전문가 유형 값 가져오기 (여러 개 선택 가능할 경우 배열로 설정)
	searchFilter.adviserTypes = $('input[name="adviser-type"]:checked').map(function() {
		return $(this).val();
	}).get();
...
// 검색 요청 함수
function getAdviseList(searchFilter) {
	...
	// api 요청
	$.ajax({
		url: '/admin/api/advise/getAdviseList',
		method: 'POST',
		contentType: 'application/json',
		data: JSON.stringify(searchFilter),
		success: function(response) {
			console.log('getAdviseList response ', response)
			resultUpdate(response.adviseList); // 검색결과 업데이트
			pagination(response.pageInfo); // 페이지네이션 업데이트
			$('#advise-all').prop('checked', false); // 결과창 전체 체크 버튼 해제
		},
...
// 검색결과 화면 업데이트 함수
function resultUpdate(adviseList) {
	var rawList = `
		<div id="loading-spinner" style="display: flex;">
		    <div class="spinner"></div>
		</div>
	`;
	if (adviseList && adviseList.length > 0) {
		// 검색 결과가 있을 경우
		adviseList.forEach(function(advise, index) {
			rawList += resultRaw(advise, index);
		});
	} else {
		// 검색 결과가 없을 경우
		rawList += `
...
// 검색 결과 HTML
function resultRaw(advise, index) {
	...
	let resultRaw = `
		<div class="result-body-raw">
            <div class="result-body-column column1" style="width: 40px;">
                <input type="checkbox" name="advise" class="advise" data-id="${advise.adviseIdx}" data-idx="${advise.adviseIdx}">
            </div>
            <div class="result-body-column column2" style="width: 60px;">
                <span>${startIndex + index}</span>
            </div>
            <div class="result-body-column column3" style="width: 100px;">
                <span>${advise.questionUserId}</span>
            </div>
            <div class="result-body-column column4" style="width: 70px;">
                <span>${resultTp(advise.questionUserTp)}</span>
            </div>
            <div class="result-body-column column5" style="width: 90px;">
                <span>${advise.adviserUserId}</span>
            </div>
            <div class="result-body-column column6" style="width: 230px;">
                <span>${resultTp(advise.adviserUserTp)}</span>
            </div>
            <div class="result-body-column column7" style="width: 140px;">
				<span>${escapeHtml(advise.adviseTitle)}</span>
			</div>
            <div class="result-body-column column8" style="width: 100px;">
				<span>${adviseStatusNm(advise.adviseStatusCd)}</span>
			</div>
            <div class="result-body-column column9" style="width: 90px;">
				<span>${formatDate(advise.regDt)}</span>
	        </div>
			<div class="result-body-column column10 fixed" style="width: 70px;">
	            <button class="info-btn" data-idx="${advise.adviseIdx}">
					<span class="blue">상세</span>
				</button>
	        </div>
	        <div class="result-body-column column11 fixed" style="width: 70px;">
				<button class="del-btn" data-advise-idx="${advise.adviseIdx}">
...
$('#result-body').on('click', '.info-btn', function() {
	// 선택한 자문 인덱스
	const adviseIdx = $(this).data('idx');
	
	// URL
	const url = '/admin/adviseDetail?adviseIdx=' + adviseIdx;
