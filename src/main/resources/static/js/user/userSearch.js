...
function resultRaw(user, index) {
	...
	let resultRaw = `
		<div class="result-body-raw">
            ...
            <div class="result-body-column column6" style="width: 160px;">
		...
		if (user.subscription == 'STTCD001') {
			resultRaw += `<span>O</span>`;	
		} else {
			resultRaw += `<span>X</span>`;
		}
		...
		resultRaw += `
	        ...
	        <div class="result-body-column column12 fixed" style="width: 70px;">
				<button class="del-btn" data-user-idx="${user.userIdx}" data-user-id="${user.userId}">
					<span class="red">탈퇴</span>
				</button>
	        </div>
        </div>
	`;
...
$('#subscription-btn').on('click', function() {
	console.log('click')
	$('.subscription-container').toggleClass('open');
});

// 구독 설정 버튼 밖에 클릭 시 닫기
$('#wrap').on('click', function(event) {
	if (!$(event.target).closest('.subscription-container, #subscription-btn').length) {
		$('.subscription-container').removeClass('open');
	}
});

// 선택한 회원 구독 설정
$('.subscription-li').on('click', function() {
	// 선택된 체크박스를 가져옴
	...
	// 구독설정할 값 저장
	const statusCd = $(this).data('subscription');
	
	// 텍스트 가져오기
	const subscriptionText = $(this).find('.subscription-li-text').text();

	let confirmed = confirm(`선택한 회원들을 구독 ${subscriptionText} 하시겠습니까?`);
	...
	selectedUsers.each(function() {
		...
		// 객체에 추가
		userIdxList.userIdx.push(userIdx);
		userIdxList.userId.push(userId);
	});


	console.log('구독 설정할 회원들: ', userIdxList);
	
	$('.subscription-container').removeClass('open');

	// 삭제 요청
	subscriptionUser(userIdxList);
});

// 구독 설정 API
function subscriptionUser(userIdxList) {
	$.ajax({
		url: '/admin/api/user/subscriptionUser',
		type: 'POST',
		contentType: 'application/json',
		data: JSON.stringify(userIdxList), // 서버에 전송할 데이터
		success: function(response) {
			alert(response.message);
			getUserList(searchFilter); // 검색 요청
		},
		error: function(error) {
			console.log('Error: ', error);
		}
...
$('#result-body').on('click', '.del-btn', function() {
	...
	showSendMail(userList, 'deleteUserOne');
...
$('#all-del-btn').on('click', function() {
	...
	showSendMail(userList, 'deleteUserGroup');
